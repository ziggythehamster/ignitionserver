...
'
' $Id: modSox.bas,v 1.4 2004/09/12 04:00:56 ziggythehamster Exp $
'
'
...
End If
Dim Socket As clsSox
Set Socket = New clsSox
...
Public Function GetAcceptClass(ByVal lngSocket As Long) As clsSox
...
Public Sub Sox_Connect(insox As Long)
On Error Resume Next
'we connected!
Dim A As Long
Dim F As Long
...
If StopIS = True Then
  ...
  Sockets.SendData "MDIE " & TheNumber & vbCrLf
  InternalDebug "Sent it..."
  For A = 1 To 100000000: DoEvents: Next A
  Sockets.CloseSck
  Terminate
ElseIf RestartIS = True Then
  InternalDebug "Restarting..."
  F = FreeFile
  Randomize Timer
  TheNumber = Int(Rnd * 1000)
  Open App.Path & "\monitor.id" For Output As F
  Print #F, TheNumber
  Close #F
  InternalDebug "Restarting with ID " & TheNumber
  Sockets.SendData "MRESTART " & TheNumber & vbCrLf
  InternalDebug "Sent it..."
  For A = 1 To 100000000: DoEvents: Next A
  Sockets.CloseSck
  Terminate
ElseIf RehashIS = True Then
  InternalDebug "Rehashing..."
  F = FreeFile
  Randomize Timer
  TheNumber = Int(Rnd * 1000)
  Open App.Path & "\monitor.id" For Output As F
  Print #F, TheNumber
  Close #F
  InternalDebug "Rehashing with ID " & TheNumber
  Sockets.SendData "MREHASH " & TheNumber & vbCrLf
  InternalDebug "Sent it..."
  For A = 1 To 100000000: DoEvents: Next A
  InternalDebug "closing sock"
  Sockets.CloseSck
  Terminate
Else
  Sockets.CloseSck
...
Public Sub Sox_DataArrival(msglen As Long)
On Error Resume Next
Dim StrMsg As String
Dim ProcMsg() As String
Sockets.GetData StrMsg, vbString
InternalDebug "Data: " & StrMsg
ProcMsg = Split(StrMsg, " ")
If UBound(ProcMsg) > 0 Then
  If ProcMsg(1) = "421" Then
    Sockets.CloseSck
    Terminate
  End If
End If
