...
; $Id: ignitionServer.nsi,v 1.21 2005/06/05 23:44:40 ziggythehamster Exp $
...
!define PRODUCT_VERSION_REVISION "7"
!define PRODUCT_VERSION_BUILD "20050602"
;;undefine the following when producing releases
!define PRODUCT_CVS
!ifdef PRODUCT_CVS
  !define PRODUCT_VERSION "${PRODUCT_VERSION_MAJOR}.${PRODUCT_VERSION_MINOR}.${PRODUCT_VERSION_REVISION}-CVS"
!else
  !define PRODUCT_VERSION "${PRODUCT_VERSION_MAJOR}.${PRODUCT_VERSION_MINOR}.${PRODUCT_VERSION_REVISION}"
!endif
!define PRODUCT_VERSION_FULL "${PRODUCT_VERSION_MAJOR}.${PRODUCT_VERSION_MINOR}.${PRODUCT_VERSION_REVISION}.${PRODUCT_VERSION_BUILD}"
...
!define PRODUCT_SOFTWARE_REGKEY "Software\The Ignition Project\ignitionServer"
...
; Language Stuff
LangString DESC_SEC_CORE      ${LANG_ENGLISH} "Install ignitionServer Core Components"
LangString DESC_SEC_MONITOR   ${LANG_ENGLISH} "Install ignitionServer Monitor, for monitoring and controlling the server"
LangString DESC_SEC_PASSCRYPT ${LANG_ENGLISH} "Install ignitionServer PassCrypt for ease in encrypting passwords"
LangString DESC_SEC_MIGWIZ    ${LANG_ENGLISH} "Install the IRCXpro Migration Wizard, to easily convert your IRCXpro settings to settings compatible with ignitionServer"
;;--future--
LangString DESC_SEC_CONFWIZ   ${LANG_ENGLISH} "Install ignitionServer Configuration Wizard, to easily configure your server."
LangString DESC_SEC_REMOTEADM ${LANG_ENGLISH} "Install ignitionServer Remote Administration to remotely administer your server."

; MUI end ------

Name "${PRODUCT_NAME} ${PRODUCT_VERSION}"
!ifdef PRODUCT_CVS
  OutFile "${PRODUCT_NAME}_${PRODUCT_VERSION_FULL}_Setup.exe"
!else
...
Section -"Core" SEC_CORE
  ;; if iS is installed already, ask
  ;; This is the best solution, but it will still overwrite
  ;; stuff from older installers (0.3.6 and earlier)
  ;; it's an improvement over what we have now, but
  ;; they won't notice until we upgrade from 0.3.7 to the
  ;; next version.
  ReadRegStr $0 HKLM "${PRODUCT_SOFTWARE_REGKEY}" ""
  IfErrors +3 0
    MessageBox MB_ICONEXCLAMATION|MB_YESNO|MB_DEFBUTTON2 "ignitionServer appears to already be installed. Are you sure you want to continue the installation? Your configuration files may be overwritten if you haven't backed them up!" IDYES +2
    Quit
  SetOutPath "$INSTDIR"
  SetOverwrite on
...
Section "PassCrypt" SEC_PASSCRYPT
  SetOutPath "$INSTDIR"
  SetOverwrite on
  File "..\PassCrypt.exe"
...
Section "IRCXpro Migration Wizard" SEC_MIGWIZ
  SetOutPath "$INSTDIR"
  SetOverwrite on
  File "..\migwiz.exe"
  File "..\migwiz.cftpl"
...
Section -Post
  ...
  WriteRegStr HKLM "${PRODUCT_SOFTWARE_REGKEY}" "" "$INSTDIR"
  ...
  WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "HelpLink" "http://www.ignition-project.com/forum/"
...
!insertmacro MUI_DESCRIPTION_TEXT ${SEC_MONITOR}   $(DESC_SEC_MONITOR)
!insertmacro MUI_DESCRIPTION_TEXT ${SEC_PASSCRYPT} $(DESC_SEC_PASSCRYPT)
!insertmacro MUI_DESCRIPTION_TEXT ${SEC_MIGWIZ}    $(DESC_SEC_MIGWIZ)
...
Section Uninstall
  ...
  DeleteRegKey HKLM "${PRODUCT_SOFTWARE_REGKEY}"
  SetAutoClose true
SectionEnd
